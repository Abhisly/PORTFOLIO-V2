...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  // Memoize random properties to prevent seizure-inducing flickering 
  // when text updates rapidly during loading
  const cellConfigs = React.useMemo(() => {
    return Array.from({ length: 300 }).map(() => ({ // 300 is large enough for normal strings
  ...
  return (
    <section
      className="w-[85vw] max-w-4xl bg-black/40 border border-white/10 grid p-2 sm:p-4 md:p-6 gap-1 sm:gap-1.5 md:gap-2 rounded-[15px] sm:rounded-[20px]"
      style={{
    ...
    >
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        const config = cellConfigs[index % cellConfigs.length];
        const shouldFlash = !isHighlighted && config.shouldFlash;

        return (
          <div
            key={index}
            className={cn(
              `border border-white/5 h-full w-full aspect-square rounded-[4px] sm:rounded-[3px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-white/5" : ""
            )}
            style={
              {
